...
module.exports = createCoreController('api::product-variant.product-variant', ({ strapi }) => ({
  ...
  async create(ctx) {
    try {
      ...
      if (!product) {
        return strapi.config.functions.response.badRequest(ctx, 'Orphaned product variant');
      }

      performCleanups(productVariantBody);

      const create = await strapi.services['product-variant'].createWithRelations(
        productVariantBody
      );
      if (create) {
  ...
  async update(ctx) {
    try {
      ...
      if (found) {
        const update = await strapi.services['product-variant'].updateWithRelations(
          productVariantBody
        );
        if (update) {
